...
export default function RepCartPage({ params }: { params: Params }) {
  ...
  if (liveCart.length === 0 && !submitted && !pending) {
    return (
      <div>
        <RepHeader
          title="Сагс"
          backHref={`/rep/stores/${id}/catalog`}
          cartScope={scope}
        />
        <div className="px-4 py-16 text-center max-w-md mx-auto">
          ...
          <p className="text-sm text-muted-foreground mb-6">
            Каталогоос бараа сонгоно уу.
          </p>
          <Button asChild>
            <Link href={`/rep/stores/${id}/catalog`}>Каталог руу</Link>
          </Button>
        </div>
      </div>
    );
  ...
  return (
    <div>
      <RepHeader
        title="Сагс"
        subtitle={`${qty} ширхэг бараа`}
        backHref={`/rep/stores/${id}/catalog`}
        cartScope={scope}
      />

      <main className="px-3 sm:px-4 py-4 max-w-2xl mx-auto pb-32">
          ...
          {cart.map((item) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <Link
                  href={`/rep/stores/${id}/catalog/${item.product_id}`}
                ...
                <div className="mt-2 flex items-center gap-2">
                  <div className="inline-flex items-center border rounded-full">
                    <button
                      type="button"
                      onClick={() => updateQty(item.product_id, item.qty - 1, scope)}
                      aria-label="Хасах"
                  ...
                  <button
                    type="button"
                    onClick={() => removeFromCart(item.product_id, scope)}
                    aria-label="Хасах"
